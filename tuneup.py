...
def check_chi(cfg_dict, qi=0, im=None, span=7, npts=301, plot=False, check_f=False):
    ...
    start = freq-4.5
    center = start + span/2
    chi = meas.ResonatorSpectroscopyExperiment(cfg_dict, qi=qi, params={'span':span, 'center':center, 'npts':npts, 'rounds':5, 'relax_delay':15}, go=False, check_e=True)
    chi.go(analyze=True, display=False, progress=True, save=True)

    if check_f: 
        chif = meas.ResonatorSpectroscopyExperiment(cfg_dict, qi=qi, params={'span':span, 'center':center, 'npts':npts, 'rounds':5, 'relax_delay':15}, go=False, check_e=True, check_f=True)
        chif.go(analyze=True, display=False, progress=True, save=True)

    rspec = meas.ResonatorSpectroscopyExperiment(cfg_dict, qi=qi, params={'span':span, 'center':center, 'npts':npts, 'rounds':2, 'relax_delay':15}, go=False, check_e=False)
    ...
    ax[0].legend()
    ax[0].axvline(x=xpts_chi[arg], color='k', linestyle='--')  # Add vertical line at selected point
...
def measure_temp(cfg_dict, qi, npts=14, reps=None, rounds=None, chan=None):
    
    rabief = meas.AmplitudeRabiExperiment(cfg_dict, qi=qi, pulse_ge=True, checkEF=True)
    rabief_nopulse = meas.AmplitudeRabiExperiment(cfg_dict, qi=qi, params={'npts':npts}, pulse_ge=False, checkEF=True, style='temp')
    
    # To measure temperature, use fewer points to get more signal more quickly 
    fge = 1e6*rabief.cfg.device.qubit.f_ge[qi]
    if chan is None:
        qubit_temp = 1e3*cs.h*fge/(cs.k*np.log(rabief.data['best_fit'][0]/rabief_nopulse.data['best_fit'][0]))
        population = rabief_nopulse.data['best_fit'][0]/rabief.data['best_fit'][0]
    else:
        qubit_temp = 1e3*cs.h*fge/(cs.k*np.log(rabief.data[chan][0]/rabief_nopulse.data[chan][0]))
        population = rabief_nopulse.data[chan][0]/rabief.data[chan][0]

    fig, ax= plt.subplots(1,1)
    i_best= str(rabief.data['i_best'])[2:-1]
    ax[0].plot(rabief.data['xpts'], rabief.data[i_best]-np.min(rabief.data[i_best]), label='ge Pulse')
    ax[0].set_ylabel('Rabei ge Pulse')
    #plt.legend()
    axt = plt.twinx()
    axt.plot(rabief_nopulse.data['xpts'], rabief_nopulse.data[i_best]-np.min(rabief_nopulse.data[i_best]), label='No Pulse', color='tab:orange')
    ...
    ax[0].set_title = f'Qubit {qi} Temperature: {qubit_temp:0.2f} mK, Population: {population:0.2g}'
...
def recenter_smart(qi, cfg_dict, start='T2r', freq=0.3, max_err=0.45, min_r2=0.1, max_t1=500):
    ...
    scans = [meas.PulseProbeSpectroscopyExperiment, cfg.meas.PulseProbeSpectroscopyExperiment, meas.PulseProbeSpectroscopyExperiment, meas.RamseyExperiment,meas.RamseyExperiment, meas.RamseyExperiment] 
    ...
    while i<ntries and not all_done: 
        ...
        prog = run_scan_level(qi, cfg_dict, scan_info, level,min_r2=min_r2, max_t1=max_t1)
        if prog.status:
            if level>2: 
...
def find_spec(qi, cfg_dict, start='coarse', max_err=0.45, min_r2=0.1, max_t1=500):
    ...
    scans = [meas.PulseProbeSpectroscopyExperiment, meas.PulseProbeSpectroscopyExperiment, meas.PulseProbeSpectroscopyExperiment] 
...
def run_scan_level(qi, cfg_dict, scan_info, level, min_r2=0.1, max_t1=500):
    ...
    prog = scan_info['scans'][level](cfg_dict,qi, params=scan_info['params'][level], min_r2=min_r2)
