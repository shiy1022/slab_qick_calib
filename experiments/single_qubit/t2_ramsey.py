...
class RamseyExperiment(Experiment):
    ...
    def analyze(self, data=None, fit=True, fit_twofreq=False,debug=False, **kwargs):
        ...
        if fit:
            ...
            print('R2:', r2)
            data['r2']=r2
            print('fit_err:', np.mean(np.abs(t2r_fit_err/t2r_fit)))
            data['fit_err']=t2r_fit_err

            data['best_fit'] = data[i_best]
    ...
    def display(self, data=None, fit=True, fit_twofreq=False,debug=False, **kwargs):
        ...
        plt.subplot(111,title=f"{title} (Ramsey Freq: {self.cfg.expt.ramsey_freq:.3f} MHz)",
        ...
        if fit:
            ...
            if isinstance(p, (list, np.ndarray)): 
                ...
                #print(f'T2 Ramsey from fit I [us]: {p[3]:.3}')
        if debug: 
