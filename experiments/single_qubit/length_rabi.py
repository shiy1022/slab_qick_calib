...
class LengthRabiProgram(AveragerProgram):
    ...
    def initialize(self):
        ...
        self.pi_test_sigma = self.us2cycles(
            cfg.expt.length_placeholder, gen_ch=self.qubit_chs[qTest]
        )
        ...
        if self.checkZZ:
            self.add_gauss(
                ...
                name="pi_qubitA",
                sigma=self.pisigma_ge_qA,
                length=self.pisigma_ge_qA * 4,
            )
        if self.checkEF:
            self.add_gauss(
                ch=self.qubit_chs[qTest],
            ...
            if "sigma_test" not in self.cfg.expt:
                if self.cfg.expt.checkZZ:
                    self.cfg.expt.sigma_test = (
                        self.cfg.device.qubit.pulses.pi_Q1_ZZ.sigma[qZZ]
                    )
