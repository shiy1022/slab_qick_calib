...
def hist(data, plot=True, span=None, ax=None, verbose=False, qubit=0):
    ...
    if plot:
        ...
        ax[1].plot(
            data["vhg"], data["histg"], ".-", color=blue, markersize=0.5, linewidth=0.3
        )
        ax[1].fill_between(data["vhg"], data["histg"], color=blue, alpha=0.3)
        ax[1].fill_between(data["vhe"], data["histe"], color=red, alpha=0.3)
        ax[1].plot(
            data["vhe"], data["histe"], ".-", color=red, markersize=0.5, linewidth=0.3
        )
        ax[1].plot(
            data["vhg"], gaussian(data["vhg"], 1, *data["paramsg"]), "k", linewidth=1
        )
        ax[1].plot(
            data["vhe"],
            excited_func(
...
class RepMeasExperiment(QickExperiment):
    ...
    def analyze(self, data=None, span=None, verbose=False, **kwargs):
        ...
        params, _ = hist(data=data, plot=False, span=span, verbose=verbose)
        data.update(params)
        try:
