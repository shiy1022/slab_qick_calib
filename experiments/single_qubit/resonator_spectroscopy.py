...
class ResSpec(QickExperiment):
    ...
    def __init__(
        ...
        if (
            params["length"] > 100
        ):  # Can't be set directly to be greater than 100 on dynamic generator; may need to adapt for other generators.
        ...
        if go:
            if style == "coarse":
                ...
                super().run(
                    display=display, progress=progress, save=save, analyze=analyze
                )
    ...
    def acquire(self, progress=False):
        ...
        if not self.cfg.expt.loop:
            ...
            exp = QickExperimentLoop(
                cfg_dict=cfg_dict, prefix="dummy", progress=progress, qi=q
            )
            ...
            if self.cfg.expt.phase_const:
                ...
                self.cfg.expt.span = (
                    np.round(self.cfg.expt.span / df / (self.cfg.expt.expts - 1))
                    * df
        ...
        **kwargs,
        ...
        if peaks:
            ...
            for i in range(len(coarse_peaks)):
                peak = coarse_peaks[i]
                ax[0].axvline(
                    data["freq"][peak], linestyle="--", color="0.2", linewidth=0.5
                )
                ax[1].axvline(
                    data["freq"][peak], linestyle="--", color="0.2", linewidth=0.5
                )
        ...
        **kwargs,
        ...
        if peaks:
            ...
            for i in range(num_peaks):
                peak = peak_indices[i]
                ax[0].axvline(
                    data["freq"][peak], linestyle="--", color="0.2", linewidth=1
                )
                ax[1].axvline(
                    data["freq"][peak], linestyle="--", color="0.2", linewidth=1
                )
        ...
        if self.status:
            ...
            if freq:
                config.update_readout(
                    cfg_file, "frequency", self.data["freq_min"], qi, verbose=verbose
                )

            # Update resonator linewidth if valid
            if self.data["kappa"] > 0:
                config.update_readout(
                    cfg_file, "kappa", self.data["kappa"], qi, verbose=verbose
                )

            # Update Q factors if not in fast mode
            if not fast:
                config.update_readout(
                    cfg_file, "qi", self.data["fit"][1], qi, verbose=verbose
                )
                config.update_readout(
                    cfg_file, "qe", self.data["fit"][2], qi, verbose=verbose
                )
...
class ResSpecPower(QickExperiment2DSimple):
    ...
    def __init__(
        ...
        params["start"] = (
            self.cfg.device.readout.frequency[qi] - params["span"] / 2 - params["f_off"]
        )
    ...
    def acquire(self, progress=False):
        ...
        if self.cfg.expt.get("log", False):
            ...
            gain_pts = self.cfg.expt.max_gain * rat ** np.arange(
...
class ResSpec2D(QickExperiment2DSimple):
    ...
    def acquire(self, progress=False):
        ...
        phs = (
            self.data["phases"] - self.cfg.device.readout.phase_inc * self.data["xpts"]
        )
        phs = np.unwrap(phs)
...
def get_homophase(params):
    ...
    flist_lin = (
        -np.arange(nlin, 0, -1) * df / N * 3 + params["center"] - params["span"] / 2
    )
    flist_linp = (
