...
class T1Cont2QExperiment(QickExperiment2Q):
    ...
    def acquire(self, progress=False, get_hist=True):
        ...
        if 'active_reset' in self.cfg.expt and self.cfg.expt.active_reset:
            final_delay = max([self.cfg.device.readout.readout_length[q] for q in self.cfg.expt.qubit])
        else:
            final_delay = max([self.cfg.device.readout.final_delay[q] for q in self.cfg.expt.qubit])
        
        prog = T1Cont2QProgram(soccfg=self.soccfg, final_delay=final_delay, cfg=self.cfg)
    ...
    def display(self, data=None, fit=True, plot_all=False, ax=None, show_hist=True, rescale=False, savefig=True, **kwargs):
        if data is None:
            data = self.data
        ...
        if self.cfg.expt.active_reset:
            n_reset = 3
            pulse_length = (self.cfg.expt.readout * (self.cfg.expt.n_g + n_reset * (self.cfg.expt.n_e + n_t1)) + 
                           self.cfg.expt.wait_time * n_t1 + nexp * pi_time)
        else:
            pulse_length = (self.cfg.expt.readout * nexp + self.cfg.expt.wait_time * n_t1 + 
                           self.cfg.expt.final_delay * (self.cfg.expt.n_e + n_t1) + nexp * pi_time)
        ...
        if show_hist:
            ...
            for q in range(len(self.cfg.expt.qubit)):
                axes_hist[q].hist(data[f'avgi_e_{q}'].flatten(), bins=50, alpha=0.6, color='r', 
                                label='Excited State', density=True)
                axes_hist[q].hist(data[f'avgi_g_{q}'].flatten(), bins=50, alpha=0.6, color='b', 
                                label='Ground State', density=True)
        ...
        for q in range(len(self.cfg.expt.qubit)):
            ...
            axes_proc[0].plot(times, smoothed_t1_data, 'k.-', linewidth=0.1, markersize=m, 
                             label='Smoothed T1 Data')
            axes_proc[1].plot(times, smoothed_g_data, 'k.-', linewidth=0.1, markersize=m, 
                             label='Smoothed g Data')
            axes_proc[2].plot(times, smoothed_e_data, 'k.-', linewidth=0.1, markersize=m, 
            ...
            axes_proc[3].plot(times, pt1, 'k.-', linewidth=0.1, markersize=m, 
                             label='Normalized T1')
            axes_proc[3].axhline(np.exp(-1), color='r', linestyle='--', label='$e^{-1}$')
            ...
            ax_comb.plot(times, smoothed_t1_data, 'k.-', linewidth=0.1, markersize=m, 
                        label='T1 Data')
            ax_e = ax_comb.twinx()
