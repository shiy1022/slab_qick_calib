...
class QickExperiment(Experiment):
    def __init__(self, cfg_dict=None, prefix='QickExp', progress=None, qi=0):
        ...
        self.reps = int(self.cfg.device.readout.reps[qi]*self.cfg.device.readout.reps_base)
        self.rounds = int(self.cfg.device.readout.rounds[qi]*self.cfg.device.readout.rounds_base)
    ...
    def analyze(self, fitfunc=None, fitterfunc=None, data=None, fit=False, **kwargs):
        ...
        for i, ydata in enumerate(ydata_lab):
            data['fit_' + ydata], data['fit_err_' + ydata], data['fit_init_' + ydata] = fitterfunc(data['xpts'][1:-1], data[ydata][1:-1], fitparams=None)
    ...
    def display(self, data=None,ax=None,plot_all=False,title='', xlabel='', fit=True, show_hist=False,fitfunc=None,captionStr=[],var=[],debug=False, **kwargs):
        ...
        for i, ydata in enumerate(ydata_lab):
            ...
            if fit:
                ...
                for j in range(len(captionStr)):
                    if j>0:
                        caption+='\n'
                    caption+=(captionStr[j].format(val=p[var[j]], err=np.sqrt(pCov[var[j],var[j]])))
                ax[i].plot(data["xpts"][1:-1], fitfunc(data["xpts"][1:-1], *p), label=caption)
            ...
            if debug: 
                ...
                print(pinit)
                ax[i].plot(data["xpts"], fitfunc(data["xpts"], *pinit), label='Initial Guess')
    ...
    def run(self, progress=True, analyze=True, display=True, save=True, min_r2=0.9, max_err=0.1):
        if min_r2 is None: 
            min_r2 = 0.1
        if max_err is None:
            max_err = 0.5
        ...
        if 'fit_err' in self.data and 'r2' in self.data and self.data['fit_err'] < max_err and self.data['r2'] > min_r2:
            self.status=True
...
class QickExperimentLoop(QickExperiment):
    ...
    def acquire(self, prog_name, x_sweep, progress=True, hist=False):
        ...
        for i in tqdm(xvals, disable=not progress):
            ...
            prog = prog_name(soccfg=self.soccfg, cfg=self.cfg)
            avgi, avgq = prog.acquire(self.im[self.cfg.aliases.soc], threshold=None, load_pulses=True, progress=False) 
    ...
    def display(self, data=None,ax=None,plot_all=False,title='', xlabel='', fit=True, show_hist=False,fitfunc=None,captionStr=[],var=[],debug=False):
        super().display(data=data,ax=ax,plot_all=plot_all,title=title, xlabel=xlabel, fit=fit, show_hist=show_hist,fitfunc=fitfunc,captionStr=captionStr,var=var,debug=debug)
        pass
    
    def run(self, progress=True, analyze=True, display=True, save=True, min_r2=0.9, max_err=0.1):
        return super().run(progress=progress, analyze=analyze, display=display, save=save, min_r2=min_r2, max_err=max_err)
...
class QickExperiment2D(QickExperimentLoop): 
    ...
    def display(self, data=None,ax=None,plot_both=False,plot_amps=False, title='', xlabel='', ylabel='', **kwargs):
        ...
        for i, ydata in enumerate(ydata_lab):
            ax[i].pcolormesh(x_sweep, y_sweep, data[ydata], cmap='viridis', shading='auto')
    ...
    def run(self, progress=True, analyze=True, display=True, save=True, min_r2=0.9, max_err=0.1):
        super().run(progress=progress, analyze=analyze, display=display, save=save, min_r2=min_r2, max_err=max_err)
...
class QickExperiment2DLoop(QickExperiment2D): 
    ...
    def display(self, data=None,ax=None,plot_both=False,title='', xlabel='', ylabel='', **kwargs):
        super().display(data=data,ax=ax,plot_both=plot_both,title=title, xlabel=xlabel, ylabel=ylabel)
