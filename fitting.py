...
def get_r2(xdata: np.ndarray, ydata: np.ndarray, fitfunc: Callable, fit_params: List[float]) -> float:
    """
...
def fourier_init(xdata: np.ndarray, ydata: np.ndarray, debug: bool = False) -> Tuple[float, float]:
    """
...
def validate_bounds(fitparams: List[float], bounds: Tuple[List[float], List[float]]) -> List[float]:
    """
...
def generic_fit(
    ...
    try:
        if bounds:
            pOpt, pCov = sp.optimize.curve_fit(fitfunc, xdata, ydata, p0=fitparams, bounds=bounds)
        else:
...
def fittwofreq_decaysin(
    ...
    if fitparams[3] is None: fitparams[3] = max(xdata) - min(xdata)  # decay0
    if fitparams[4] is None: fitparams[4] = np.mean(ydata)  # y00
    if fitparams[5] is None: fitparams[5] = xdata[0]  # x00
    if fitparams[6] is None: fitparams[6] = 1  # yscale1
    if fitparams[7] is None: fitparams[7] = 1/10  # freq1
    if fitparams[8] is None: fitparams[8] = 0  # phase_deg1
    if fitparams[9] is None: fitparams[9] = 0  # y01
...
def adiabatic_amp(t: np.ndarray, amp_max: float, beta: float, period: float) -> np.ndarray:
    """
